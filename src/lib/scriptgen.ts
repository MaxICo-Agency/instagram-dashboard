...
export async function generateScript(
  ...
  if (!KEY) return { error: "Немає OPENAI_API_KEY на сервері." };
...
• Хук: ${qBetter ? "починати з питання (більше залученості)" : "починати з провокаційного твердження"}
• Стиль мовлення: ${casualBetter ? "розмовний зі сленгом — дає більше переглядів" : "нейтральний діловий — працює краще"}
• Слова-магніти для хука: ${hookWords || "—"}
• Слова тем, що заходять: ${winWords || "—"}
• Сер. ER кращих: ${patterns.avgER}%`;

  const system = `Ти — персональний скрипт-райтер блогера @max.shapoval (Instagram Reels, ніша: маркетинг / таргет / SMM / AI для бізнесу). Ти глибоко вивчив його дані й точно знаєш, що працює в цієї аудиторії.

${styleProfile}

Твоя задача — писати скрипти, які звучать на 100% як він сам, не як ШІ. Пиши УКРАЇНСЬКОЮ.`;

  const user = `Ось мої топ-рилси — вивчи мій стиль, ритм, структуру фраз, переходи:
...
4. ${casualBetter ? "Розмовна мова, можна сленг — в автора це працює." : "Розмовно, але без зайвого мату."}
5. ${hookWords ? "Вживай слова-магніти в хуку, де доречно: " + hookWords : ""}
6. Копіюй ритм і переходи з прикладів вище.
